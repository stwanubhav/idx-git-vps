...
# Function to display colored output with improved formatting
print_status() {
    local type=$1
    local message=$2
...
create_new_vm() {
    ...
    # OS Selection with improved formatting
    print_status "INFO" "Select an operating system:"
    echo "╭──────────────────────────────────────────────────────────────────────────────╮"
    local os_options=()
    local i=1
...
start_vm() {
    ...
    if load_vm_config "$vm_name"; then
        ...
        draw_line
        echo "│ SSH Access: ssh -p $SSH_PORT $USERNAME@localhost                          │"
...
edit_vm_config() {
    ...
    if load_vm_config "$vm_name"; then
        ...
        while true; do
            ...
            echo "│  7) Memory (RAM)    │  8) CPU Count       │  9) Disk Size                   │"
    ...
    if load_vm_config "$vm_name"; then
        if is_vm_running "$vm_name"; then
            ...
            if [[ -n "$qemu_pid" ]]; then
                ...
                # Show disk usage
                echo "│ VM Disk Usage:"
                df -h "$IMG_FILE" 2>/dev/null | sed 's/^/│ /' || du -h "$IMG_FILE" | sed 's/^/│ /'
            else
    ...
    while true; do
        ...
        case $choice in
            ...
            8)
                ...
                ;;
            0)
                print_status "INFO" "Thank you for using Enhanced Multi-VM Manager!"
                print_status "INFO" "Created by ANUBHAV - Goodbye!"
                exit 0
                ;;
            *)
